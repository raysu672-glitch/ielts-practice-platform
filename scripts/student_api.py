...
LISTENING_BOOK_KEYS = frozenset({"listening", "listening_basic"})


def normalize_listening_book_key(value: Any) -> str:
    key = str(value or "listening").strip() or "listening"
    if key not in LISTENING_BOOK_KEYS:
        raise ValueError("book_key 无效，仅支持 listening / listening_basic")
    return key


def ensure_listening_group_progress_table(conn: Any) -> None:
    conn.execute(
        """
        CREATE TABLE IF NOT EXISTS listening_group_progress (
            student_id TEXT NOT NULL REFERENCES students(student_id),
            book_key TEXT NOT NULL DEFAULT 'listening',
            ...
            PRIMARY KEY (student_id, book_key, group_index)
        )
        """
    )
    conn.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_listening_group_progress_student
        ON listening_group_progress(student_id)
        """
    )


def load_listening_group_progress(
    conn: Any, student_id: str, book_key: str = "listening"
) -> dict[str, Any]:
    ensure_listening_group_progress_table(conn)
    key = normalize_listening_book_key(book_key)
    rows = conn.execute(
        """
        SELECT group_index, first_correct, first_total, status, updated_at
        FROM listening_group_progress
        WHERE student_id = ? AND book_key = ?
    ...
    for row in rows:
        item = dict(row)
        item["group_index"] = int(item["group_index"])
        item["first_correct"] = int(item.get("first_correct") or 0)
        item["first_total"] = int(item.get("first_total") or 0)
        item["status"] = str(item.get("status") or "pending")
        groups.append(item)
    return {"book_key": key, "groups": groups}


def upsert_listening_group_progress(
    conn: Any, student_id: str, payload: dict[str, Any]
) -> dict[str, Any]:
    ensure_listening_group_progress_table(conn)
    key = normalize_listening_book_key(payload.get("book_key") or payload.get("bookKey"))
    ...
    for raw in raw_groups:
        if not isinstance(raw, dict):
            continue
        try:
            group_index = int(raw.get("group_index", raw.get("groupIndex")))
        except (TypeError, ValueError):
            raise ValueError("group_index 无效") from None
        if group_index < 0 or group_index > 500:
            raise ValueError("group_index 超出范围")

        first_correct = int(raw.get("first_correct", raw.get("firstCorrect") or 0) or 0)
        first_total = int(raw.get("first_total", raw.get("firstTotal") or 0) or 0)
        if first_correct < 0 or first_total < 0:
            raise ValueError("首轮正确数无效")
        if first_total > 0 and first_correct > first_total:
        ...
        existing = conn.execute(
            """
            SELECT first_correct, first_total, status
            FROM listening_group_progress
            WHERE student_id = ? AND book_key = ? AND group_index = ?
            """,
            (student_id, key, group_index),
        ).fetchone()
        if existing:
            if first_total <= 0:
                first_correct = int(existing["first_correct"] or 0)
                first_total = int(existing["first_total"] or 0)
            if status == "pending" and str(existing["status"] or "") == "completed":
                status = "completed"
            conn.execute(
                """
                UPDATE listening_group_progress
                SET first_correct = ?, first_total = ?, status = ?, updated_at = ?
                WHERE student_id = ? AND book_key = ? AND group_index = ?
                """,
                (first_correct, first_total, status, now, student_id, key, group_index),
            )
        else:
            conn.execute(
                """
                INSERT INTO listening_group_progress (
                    student_id, book_key, group_index,
                    first_correct, first_total, status, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (student_id, key, group_index, first_correct, first_total, status, now),
            )
    conn.commit()
    return load_listening_group_progress(conn, student_id, key)
