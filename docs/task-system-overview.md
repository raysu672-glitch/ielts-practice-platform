...
|---------|------|----------|
| 姓名 / 学号 | | |
| 昨日任务 | 昨天已完成 / 共几条 | 昨天一条没做 → 🔴；没做完 → 🟡 |
| 昨日学习时长 | 昨天已练 / 累计总时长 | — |
| 积压 | **昨天**任务里还没做完的条数（**不含阶段测**——测不算任务量） | （不再单独判红，状态灯已由「昨日任务」决定） |
| 阶段测 | 待通过阶段测条数（单独成列，可点开看已考次数/最高分）；**累计考 ≥3 次仍未过**时显示红字 `N 项 多次未过` | — |
| 计划进度 | 各模块简写或折叠 | — |

**红黄灯只看昨天任务**（2026-09-20 简化）：昨天一条没做 → 🔴，昨天没做完 → 🟡，
昨天全做完或昨天没排任务 → 🟢，无计划/已暂停 → ⚪。其余判定全部取消。
**阶段测不算任务量**（2026-09-20 第四次修订）：昨天派了没考、或考了没过，
都不算积压、也不影响灯色——测过不了会一直挂在那儿，若还算完成率，这部分学生的
「昨日任务」永远差一条，怎么做都翻不了身。测的欠账单独看「待通过阶段测」列。
**阶段测不限重测次数**（2026-09-20 起）：学生想考几次都行，系统不做次数拦截。
**考挂只留测挂着**（第六次修订）：测没过就留在清单里等学生自己再考，
**科目任务不重排**（一度试过「考挂 → 把覆盖单元打回未完成强制重学」，模拟证明那会把学生
钉在同一批单元上打转）；学生想再学，从「学习进度」里点对应模块的学习按钮进去。
累计 ≥3 次仍不过 → 转 `N 项 多次未过`，请助教介入（见 §9.2）。

操作：点行进入「学生计划页」；支持按「有积压 / 昨日未完成 / 今日未完成」筛选。
...
| D23 | **打包器**积压 = 曾进 daily 且未完成；未装箱队首不算。**看板**积压 = **昨天**没做完的条数（D23 v4，**不含阶段测**，两者口径刻意分开） |
| D24 | 队首测挂死 **不**自动跳过、**不限**重测次数（≥3 次不过只提示助教）；测**不算任务量**；考挂只留测挂着、**科目任务不重排**（第六次修订废除自动重学） |
| D25 | 任务模式 **禁**下一课（含 P4） |
...
| 级别 | 谁看 | 看什么 |
|------|------|--------|
| **班级总览** | 助教每日扫一眼 | 昨日完成率、积压人数、待通过阶段测（**仅计划轨**） |
...
| 信号 | 建议动作 |
|------|----------|
| **昨天任务一条没做（🔴）** | 谈话或降清单量 / 调低周中分钟 |
| **昨天任务没做完（🟡）** | 提醒学生，确认卡在哪一步 |
| 积压 ≥3 条 | 减新单元或提高周末分钟；清队首卡点 |
| 阶段测同一范围失败 ≥3 次 / 队首测挂死 | **不自动跳过**；科目任务不重排（第六次修订废除自动重学），助教可移队尾 / 插复习 / 换更基础的内容（不限重测次数，学生可继续考） |
...
| 阶段测未过关 | 可重测（不限次数）；只留测挂着，科目任务不重排 | 未过关标记；≥3 次不过 → `多次未过` | 当日 |
